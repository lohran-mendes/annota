...
import type { Exam, MockExam, Question, Subject, Topic } from '@annota/shared';
...
export interface MockExamDialogData {
  mockExam?: MockExam;
  exams: Map<string, Exam>;
}

@Component({
  selector: 'annota-mock-exam-dialog',
  imports: [
    ReactiveFormsModule,
    MatDialogModule,
    MatButtonModule,
  ...
  template: `
    <h2 mat-dialog-title>{{ data.mockExam ? 'Editar Simulado' : 'Novo Simulado' }}</h2>
    <mat-dialog-content>
      <mat-tab-group>
        <mat-tab label="Informações">
          <form [formGroup]="form" class="dialog-form">
            <mat-form-field appearance="outline">
              ...
              <input matInput formControlName="name" placeholder="Ex: Simulado 1 - Matemática">
            </mat-form-field>
            ...
            <mat-form-field appearance="outline">
              ...
              <mat-select formControlName="examId" (selectionChange)="onExamChange($event.value)">
            ...
            <mat-form-field appearance="outline">
              ...
              <mat-hint>Tempo do simulado em minutos</mat-hint>
            </mat-form-field>

            <div class="published-toggle">
              <mat-slide-toggle formControlName="published">Publicado</mat-slide-toggle>
              <span class="toggle-hint">Simulados publicados ficam visíveis para os alunos</span>
            </div>
          </form>
        </mat-tab>
        ...
        <mat-tab label="Questões">
          <div class="questions-tab">
            @if (loadingQuestions()) {
            ...
            } @else if (!form.value.examId) {
              <div class="empty-state">
                <mat-icon>info_outline</mat-icon>
                <p>Selecione uma prova na aba "Informações" para ver as questões disponíveis.</p>
              </div>
            } @else {
              <!-- Filtros -->
              <div class="filters">
              ...
              <!-- Resumo -->
              <div class="summary">
                <span><strong>{{ selectedQuestionIds().size }}</strong> questões selecionadas</span>
              </div>
            }
          </div>
        </mat-tab>
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-flat-button color="primary" [disabled]="form.invalid || selectedQuestionIds().size === 0" (click)="save()">
        {{ data.mockExam ? 'Salvar' : 'Criar' }}
      </button>
    </mat-dialog-actions>
  `,
  styles: [`
    ...
    @media (max-width: 599px) {
      ...
      .published-toggle { flex-direction: column; align-items: flex-start; }
    }
  `],
})
export class MockExamDialog implements OnInit {
  readonly dialogRef = inject(MatDialogRef<MockExamDialog>);
  readonly data = inject<MockExamDialogData>(MAT_DIALOG_DATA);
  ...
  form = this.fb.group({
    name: [this.data.mockExam?.name ?? '', Validators.required],
    description: [this.data.mockExam?.description ?? ''],
    examId: [this.data.mockExam?.examId ?? '', Validators.required],
    duration: [this.data.mockExam?.duration ?? 180, [Validators.required, Validators.min(1)]],
    published: [this.data.mockExam?.published ?? false],
  });

  // Exam list for dropdown
  examList = signal<Exam[]>(Array.from(this.data.exams.values()));
  ...
  selectedQuestionIds = signal<Set<string>>(
    new Set(this.data.mockExam?.questionIds ?? []),
  );
  ...
  // Subjects available for the selected exam's questions
  availableSubjects = computed(() => {
    const examQIds = new Set(this.examQuestions().map((q) => q.subjectId));
    return this.allSubjects().filter((s) => examQIds.has(s.id));
  });

  filteredTopics = computed(() => {
    const subjectId = this.subjectFilter();
    const topics = this.allTopics().filter((t) =>
      this.examQuestions().some((q) => q.topicId === t.id),
    );
  ...
  ngOnInit() {
    forkJoin({
      ...
      next: ({ questions, subjects, topics }) => {
        ...
        // If editing, pre-filter questions for the existing examId
        const examId = this.form.value.examId;
        if (examId) {
          this.filterQuestionsForExam(examId);
        }
      },
      error: () => {},
    });
  ...
  onExamChange(examId: string) {
    // Reset question selection and filters when exam changes
  ...
  private filterQuestionsForExam(examId: string) {
    // Load questions linked to the selected exam via the exam questions endpoint
    this.loadingQuestions.set(true);
    this.examService.getExamQuestions(examId).subscribe({
      next: (res) => {
        this.examQuestions.set(res.data);
        this.loadingQuestions.set(false);
      },
      error: () => {
        // Fall back to filtering all questions if endpoint unavailable
        this.examQuestions.set(this.allQuestions());
        this.loadingQuestions.set(false);
      },
    });
  }
  ...
  save() {
    if (this.form.valid && this.selectedQuestionIds().size > 0) {
